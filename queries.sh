...
echo "$($PSQL "SELECT teams.name FROM (SELECT winner_id as team_id FROM games WHERE round = 'Eighth-Final' AND year = '2014' UNION SELECT opponent_id as team_id FROM games WHERE round = 'Eighth-Final' AND year = '2014') as finals_teams LEFT JOIN teams USING(team_id) ORDER BY name")"
